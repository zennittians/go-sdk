...
func ImportFromPrivateKey(privateKey, name, passphrase string) (string, error) {
	...
	itcAddress := address.ToBech32(crypto.PubkeyToAddress(sk.PublicKey))

	if store.FromAddress(itcAddress) != nil {
		return "", fmt.Errorf("address %s already exists", itcAddress)
	}
